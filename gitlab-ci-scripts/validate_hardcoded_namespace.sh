...
for manifest in "${MANIFEST_FILES[@]}"; do
  ...
  COMPONENT_NAMESPACE=""
  COMPONENT_APP_FILE="${COMPONENTS_DIR}/${COMPONENT_NAME}/application.yml"
  if [ -f "$COMPONENT_APP_FILE" ]; then
    COMPONENT_NAMESPACE="$(get_app_metadata "$COMPONENT_APP_FILE" "spec.destination.namespace")"
  fi

  log_subsection "🔍 Checking: $COMPONENT_NAME"
  log_info "File: $manifest"
  if [ -n "$COMPONENT_NAMESPACE" ]; then
    log_info "Allowed namespace: $COMPONENT_NAMESPACE"
  fi
  ...
  if [ ! -s "$manifest" ]; then
    ...
    while IFS=$'\t' read -r doc_index item_index api_version kind name namespace allow_namespace; do
      ...
      if [ -n "$namespace" ]; then
        if [ -n "$COMPONENT_NAMESPACE" ] && [ "$namespace" = "$COMPONENT_NAMESPACE" ]; then
          continue
        fi

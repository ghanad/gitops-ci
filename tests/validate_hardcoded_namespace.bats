...
setup() {
  ...
  mkdir -p "$OUT_DIR" "$RENDERED_DIR" "$GITOPS_COMPONENTS_DIR"
}
...
@test "validate_hardcoded_namespace: allows namespaces matching application destination" {
  mkdir -p "$GITOPS_COMPONENTS_DIR/reflector"
  cat <<'YAML' > "$GITOPS_COMPONENTS_DIR/reflector/application.yml"
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: reflector
...
YAML
  ...
  cat <<'YAML' > "$RENDERED_DIR/reflector.yaml"
apiVersion: v1
kind: ServiceAccount
metadata:
  name: reflector

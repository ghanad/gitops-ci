...
@test "sanity_prefix: Passes when all Application names match prefix" {
  export GITOPS_APPLICATION_NAME_PREFIX="cluster-a-"

  mkdir -p "$GITOPS_COMPONENTS_DIR/app1"
  echo "metadata: { name: cluster-a-prometheus }" > "$GITOPS_COMPONENTS_DIR/app1/application.yml"
...
@test "sanity_prefix: Fails when Application name does not match prefix" {
  export GITOPS_APPLICATION_NAME_PREFIX="cluster-b-"
  ...
  echo "Output: $output"
  [ "$status" -eq 1 ]
  [[ "$output" =~ "Invalid Application name" ]]
}
